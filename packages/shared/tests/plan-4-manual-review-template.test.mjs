...
test("Plan 4 template remains manual review required", () => {
  const path = fileURLToPath(new URL("../../../docs/manual-review/plan-4-review-record.template.json", import.meta.url));
  const record = validateManualReviewDecisionRecord(JSON.parse(readFileSync(path, "utf8")));
  assert.equal(record.planId, "plan-4");
