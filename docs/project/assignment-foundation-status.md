...
Multi-staff policy is explicit through the shared co-assignment policy contract. The `mode` field is the policy preset, and `allowMultipleForTargetKinds` is the explicit override list used by the `single_primary_per_patient_target` preset. Patient-room, hall-bed, and split-bed targets default to one primary manual staff assignment and validation warns when multiple staff are placed on the same target. Explicit support-area and zone targets may show multiple manual staff labels when modeled as assignment targets. The `allow_multiple_manual_staff` preset allows multiple manual staff on every assignment target kind. The overlay must not collapse additional staff silently; it shows the primary visible label plus a count for additional manual staff on the target.
