...
## Summary
Floorplan authoring foundation evidence for stage room-type-editing.

## Files Changed
See repository diff for shared floorplan authoring modules, web authoring controls, and local evidence.

## Commands Run
- node scripts/check-floorplan-authoring.mjs --stage room-type-editing --allow-partial --issue 273

## Tests Passed/Failed
Recorded in mapped local command output. Acceptance gates are captured separately when run.

## Evidence
- docs/verification/issues/issue-273/test-output/floorplan-authoring-gate.txt
...
GO / NO-GO for Issue 274: GO if local gates pass.
