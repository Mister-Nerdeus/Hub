...
Plan Builder Input evidence is docs-gated by `node scripts/check-docs-contracts.mjs`; the gate requires the phase evidence doc, checklist, Issue 081 defaults output, generated plan output, screenshot, validation output, commands, and closeout artifacts to exist and remain non-empty.
