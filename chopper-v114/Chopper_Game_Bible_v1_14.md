...
**Release:** 1.14 · **Date:** September 19, 2026 · **Stable document ID:** `BIB-CHOP-UE2D-001`  
**Release ID:** `CG-RELEASE-1.14` · **Target:** Unreal Engine 5.8, deterministic 2D reconstruction research  
...
> **Current decision:** **v1.14 resolves the historical repaired-MCU startup blocker at an independent research-model level, while strict arcade fidelity remains NO-GO.** The unmodified historical BAD_DUMP is still proven unusable in the pinned MAME 0.289 v1.13 run. The exact three MAME 0.131u2 repairs were then exercised independently against the supplied MCU and protected main ROMs: the repaired 8751 model reached the expected PPI initialization and deasserted Z80 BUSREQ, after which an independent Z80 model left PC=0, selected bank 3 and produced bounded work/sprite/palette/video writes across 120 nominal frame-equivalent intervals. These results are explicitly **NONCANONICAL research evidence**; no semantic/gameplay fact is promoted from them. A repaired run under the pinned MAME 0.289 binary is still pending because the native execution backend is unavailable in this turn, and the canonical MCU remains required for strict held-out verification.
...
- **`CG-LANE-HISTORICAL-MCU-REPAIR`** — exact admitted historical BAD_DUMP plus the three public MAME repairs. Current status: `INDEPENDENT_STARTUP_PROVEN_MAME_CONFIRMATION_PENDING`. The independent model proves the repaired firmware reaches the expected PPI initialization and releases Z80 BUSREQ; a native pinned-MAME repaired capture remains pending. Evidence from this lane is noncanonical and may create discovery/candidate evidence only.
...
# Current update record — v1.14 historical MCU repair and independent startup proof
...
The repaired-lane MAME 0.289 confirmation remains `NOT_RUN_ENVIRONMENT_BLOCKED` in this release, rendered title-screen fidelity is not claimed, and the canonical MCU is still mandatory for canonical held-out verification. Strict arcade fidelity therefore remains **NO-GO**.
