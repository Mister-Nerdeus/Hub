...
**Release:** 1.14-CANDIDATE · **Date:** September 19, 2026 · **Stable document ID:** `BIB-CHOP-UE2D-001`  
**Release ID:** `CG-RELEASE-1.14-CANDIDATE` · **Target:** Unreal Engine 5.8, deterministic 2D reconstruction research  
...
> **Current decision:** the research-package architecture remains valid and **strict arcade-fidelity release remains NO-GO**. The v1.13 negative run proved that the unmodified historical BAD_DUMP stalls the protected parent under MAME 0.289. A new historical-source audit now resolves the practical research-runtime blocker: MAME 0.131 first used a bounded Choplifter 8751 simulation, and MAME 0.131u2 replaced it with real 8751 execution while applying three explicit in-memory repairs to the same BAD_DUMP supplied here. v1.14-CANDIDATE encodes that repair as a fail-closed research lane. It does **not** relabel the repaired image as canonical and it cannot create VERIFIED evidence without later disjoint canonical comparison.
...
v1.14-CANDIDATE therefore adds three explicit evidence lanes:
...
- **`CG-LANE-HISTORICAL-MCU-REPAIR`** — exact admitted historical BAD_DUMP plus the three public MAME repairs. Current status: `IMPLEMENTED_PENDING_EXECUTION`. Captures are tagged `NONCANONICAL_HISTORICAL_MAME_REPAIR` and can create discovery/candidate evidence only.
...
The next executable acceptance test is a repaired `CG-PLAN-001`: the main Z80 must leave PC=0, execute nontrivial address coverage, and produce changing boot/title video/RAM state while the capture closes cleanly. Until that run is actually executed, the lane remains `IMPLEMENTED_PENDING_EXECUTION` rather than `RUNTIME_PROVEN`.
...
# Current update record — v1.13 official MAME runtime bridge and noncanonical capture
